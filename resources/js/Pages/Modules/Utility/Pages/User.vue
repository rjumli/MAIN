                            ...
                            <tr v-for="(list,index) in lists" v-bind:key="index" :class="[(list.is_active == 0) ? 'table-warnings' : '']">
                                <td>
                                    <div class="avatar-xs chat-user-img online">
                                        <span v-if="list.avatar == null" :class="'avatar-title rounded-circle bg-primary text-white'">{{list.lastname[0]}}</span>
                                        <img v-else :src="currentUrl+'/storage/'+list.avatar" alt="" class="avatar-xs rounded-circle">
